...
// 	  type: "POST",
// 	  dataType: 'json',
// 	  url: "/lib_getengagepushnotification/" + $('#pushnotificationlist').val()  ,
...
function displayPushNotification(id) {
	...
	$.ajax({
	  ...
	  success: function(data) {
	  	...
	  	if(data) {
			...
			// alert("notificationtypes : " + notificationtypes);
			$("#pushdisplaynotificationtype").html(notificationtypes);
...
$( "#editpushnotification" ).click(function(event) {
...
function showPushImageEditFirst(image_url){
	...
	imgtag = "<img src='" + src + "' style='margin-bottom: 10px;' class='img-responsive'/>";
	// alert("showPushImageEditFirst : imgtag " + imgtag);
...
$( "#savepushnotification" ).click(function(event) {
	...
	if($( "#pushname" ).val() == "") {
	    ...
	    var data = {'id': id, 'name': name, 'message': message, 'image_url': image_url, 'sound': sound, 
	    			'vibrate': vibrate , 'preload': preload, 'mb_ext': mb_ext};
	    console.log(data);
