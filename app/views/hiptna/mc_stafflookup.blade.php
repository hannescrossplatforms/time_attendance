...
.modstattitle h3{
  ...
  <body class="hipTnA">
    ...
    <script src="{{ asset('js/hiptna/hiptna.js') }}"></script>
    <!-- <script src="js/hiptna/membergraphs.js"></script>  -->
    ...
    <script>
      ...
      $( ".sendMessage" ).on( "click", function(event) {
        ...
        $.ajax({
            ...
            url: "{{ url('hiptna_sendPushNotifications'); }}",
      ...
      function showStaffTable(staffjson) {
        ...
        beginTable = '\
                <table class="table table-striped">\n\
                  ...
                  <tbody>  \n';
        $.each(staffjson, function(index, value) {

            viewbutton = '<a id=view_' + value["id"] + ' href="#" class="btn btn-default btn-sm staff-view" data-external-user-id=' + value["external_user_id"] + ' >View</a>\n';
            messagebutton = '<a id=message_' + value["id"] + ' href="#" class="btn btn-default btn-sm staff-message pagesendbutton" data-external-user-id=' + value["external_user_id"] + ' data-toggle="modal" data-target="#messageModal">Send message</a>\n';

            rows = rows + '\
                    <tr>\n\
                      <td> ' + value["surname"]  + '</td>\n\
                      <td> ' + value["firstnames"]  + '</td>\n\
                      <td> ' + value["external_user_id"]  + '</td>\n\
                      <td> ' + value["smollan_cellphone"]  + '</td>\n\
                      <td> ' + viewbutton + messagebutton + '</td>\n\
                    </tr>\n\
                    ';
        });
