...
class Picknpay extends Eloquent {
    ...
    public static function hannesTestCategoriesInner($date){
        ...
        return DB::select( DB::raw("SELECT category, created_at, sum(CAST(dwell_time AS UNSIGNED)) AS value, DATE_FORMAT(created_at, '%Y-%m-%d') AS test FROM picknpay WHERE DATE_FORMAT(created_at, '%Y-%m-%d') = '2019-06-05' GROUP BY category, created_at ORDER BY created_at");
