...
class Picknpay extends Eloquent {
    ...
    public static function fetchCategoryPerDate($category){
        ...
        return DB::select(DB::raw("SELECT sum(CAST(dwell_time AS UNSIGNED))AS value FROM picknpay WHERE category = '$category' GROUP BY category, DATE_FORMAT(created_at,'%Y-%m-%d')"));
