...
class Venue extends Eloquent implements UserInterface, RemindableInterface
{
    ...
    public function getVenuesForUser($productname = null, $showall = null, $device_type = null, $brand_id = null, $active_status = null)
    {
        ...
        */
        ...
        if (\User::isVicinity()) {
            $query->where('venues.brand_id', "=", 165);
        }
        $venues = $query->get();

        if ($user->level_code == "superadmin" || \User::isVicinity()) {
            //dd($venues);
            return $venues;
        }
